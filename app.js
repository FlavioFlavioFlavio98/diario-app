...
// SDK Gemini aggiornata (ESM per browser)
...
function startSessionTimer() {
    ...
    setInterval(() => {
        ...
        const statEl = document.getElementById('stat-session-time');
        if (statEl) statEl.innerText = `${m}:${s}`;
    }, 1000);
}
...
async function loadGlobalStats() {
    try {
        ...
        console.log("No global stats yet");
    }
}
...
async function loadDiaryForDate(dateStr) {
    ...
    onSnapshot(docRef, (snap) => {
        if (snap.exists()) {
            ...
            // ✅ Legge sempre le note vecchie (htmlContent) e supporta anche eventuale campo "content"
            const html = (data.htmlContent ?? data.content ?? "");

            if (document.activeElement && document.activeElement.id !== 'editor') {
...
async function saveData() {
    ...
    try {
        ...
        const dataToSave = {
            // ✅ Campo “storico” usato dalla tua app: NON si rompe nulla
            htmlContent: content,
            // ✅ Compatibilità extra (se qualche versione vecchia/nuova legge content)
            content: content,
...
async function searchByTag(tagName) {
    ...
    querySnapshot.forEach((docSnap) => {
        ...
        div.innerHTML = `<span>🗓️ ${docSnap.id}</span> <span>${d.stats?.words || 0} parole</span>`;
        div.onclick = () => {
            document.getElementById('tag-modal').classList.remove('open');
            document.getElementById('date-picker').value = docSnap.id;
            changeDate(docSnap.id);
        };
...
window.toggleDictation = () => {
    if (recognition) {
        document.getElementById('mic-btn').classList.contains('recording') ? recognition.stop() : recognition.start();
    } else {
        alert("Speech recognition non supportata su questo browser.");
    }
};
...
window.openQuestionsHistory = () => {
    ...
    document.getElementById('questions-list-container').innerHTML = "Funzione in sviluppo...";
};

// WALK & TALK
let walkRecognition = null;
let isWalkSessionActive = false;

window.openWalkTalk = () => document.getElementById('walk-talk-modal').classList.add('open');
window.closeWalkTalk = () => {
    stopWalkSession();
    document.getElementById('walk-talk-modal').classList.remove('open');
};
window.toggleWalkSession = () => { if (isWalkSessionActive) stopWalkSession(); else startWalkSession(); };

function startWalkSession() {
    if (!('webkitSpeechRecognition' in window)) { alert("Speech recognition non supportata."); return; }
    isWalkSessionActive = true;
    document.getElementById('walk-mic-btn').classList.add('active');
    document.getElementById('walk-status').innerText = "Ascolto...";

    walkRecognition = new webkitSpeechRecognition();
    walkRecognition.continuous = false;
    walkRecognition.lang = 'it-IT';

    walkRecognition.onresult = (e) => {
        const t = e.results[0][0].transcript;
        document.getElementById('walk-transcript').innerText = t;
...
function stopWalkSession() {
    isWalkSessionActive = false;
    document.getElementById('walk-mic-btn').classList.remove('active');
    document.getElementById('walk-status').innerText = "Stop";
    if (walkRecognition) walkRecognition.stop();
}
...
window.triggerBrainstorm = () => {
    ...
    document.getElementById('ai-actions').innerHTML = `<button class="ai-btn-small" onclick="insertPrompt('${p.replace(/'/g, "\\'")}')">Inserisci</button>`;
